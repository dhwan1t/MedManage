...
const INITIAL_AMBULANCES = [
    { id: 'AMB-204', status: 'EN_ROUTE', x: 15, y: 85, targetHospital: 'h1', eta: '5:42', caseId: 'PT-9942', severity: 'CRITICAL' },
    { id: 'AMB-188', status: 'EN_ROUTE', x: 90, y: 90, targetHospital: 'h3', eta: '12:10', caseId: 'PT-8811', severity: 'URGENT' },
    { id: 'AMB-302', status: 'AVAILABLE', x: 50, y: 50, targetHospital: null, eta: null, caseId: null, severity: null }, // Stationary
];
    ...
    useEffect(() => {
        ...
        const moveTimer = setInterval(() => {
            setAmbulances(prev => prev.map(amb => {
                ...
                const target = MOCK_HOSPITALS.find(h => h.id === amb.targetHospital);
                ...
                if (distance < 1) {
                    return { ...amb, x: target.x, y: target.y, status: 'ARRIVED', eta: 'Arrived' };
                }
        ...
        <div className="min-h-screen bg-white text-gray-900 dark:bg-gray-950 dark:text-white">
            ...
            <header className="bg-slate-800/80 backdrop-blur-md border-b border-slate-700/50 p-4 shrink-0 flex justify-between items-center z-20">
                <div>
                    ...
                    <p className="text-slate-400 dark:text-gray-500 text-xs font-bold tracking-widest mt-1 uppercase">Real-Time Ambulance Logistics & Hospital Network</p>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex gap-4 text-xs font-bold bg-slate-900/50 px-4 py-2 rounded-full border border-slate-700/50 text-slate-300">
                        <div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"></span> Available Hosp</div>
                        <div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]"></span> At Capacity</div>
                        <div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.6)]"></span> ER Only</div>
                        <div className="w-px h-4 bg-slate-700 mx-1"></div>
                        <div className="flex items-center gap-1.5"><span className="text-base leading-none drop-shadow-[0_0_6px_rgba(96,165,250,0.8)]">🚑</span> En Route</div>
                        <div className="flex items-center gap-1.5 opacity-50"><span className="text-base leading-none grayscale">🚑</span> Standby/Idle</div>
                    </div>
                </div>
            </header>
            ...
            <div className="flex-1 flex overflow-hidden relative">
                ...
                `,
                    ...
                    <svg className="absolute inset-0 w-full h-full pointer-events-none drop-shadow-[0_0_4px_rgba(96,165,250,0.5)] z-0"
                        viewBox={`0 0 100 100`}
                    ...
                    {MOCK_HOSPITALS.map((hospital) => (
                        ...
                        >
                            <div className="relative">
                                {/* Pulse Ring */}
                                <div className={`absolute -inset-2 rounded-full opacity-30 animate-ping ${getHospitalColor(hospital.status)}`}></div>
                                {/* Status Dot */}
                                <div className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-[#0f172a] shadow-lg z-10 ${getHospitalColor(hospital.status)}`}></div>
                                {/* Icon */}
                                <div className="text-3xl drop-shadow-[0_4px_8px_rgba(0,0,0,0.5)] transform transition-transform group-hover:scale-125">🏥</div>
                            </div>
                            {/* Label */}
                    ...
                    {ambulances.map((amb) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`text-2xl drop-shadow-md relative 
                  ${isHighlighted ? 'drop-shadow-[0_0_12px_rgba(250,204,21,0.8)] scale-125' : 'drop-shadow-[0_0_8px_rgba(96,165,250,0.5)]'}
                `}>
                                    🚑
                                </div>
                                <div className={`mt-1 text-[9px] font-black tracking-widest flex flex-col items-center px-1.5 py-0.5 rounded shadow-xl whitespace-nowrap border
                ...
                `}>
                                    <span>{amb.id}</span>
                                    {!isIdle && <span className="text-[8px] opacity-90">{amb.eta}</span>}
                                </div>
                            </div>
                        );
                    })}
                ...
                <div className="w-80 bg-slate-800/90 backdrop-blur-md border-l border-slate-700/50 flex flex-col z-20 shrink-0 shadow-2xl">
                    <div className="px-5 py-4 border-b border-slate-700/50 bg-slate-900/40">
                        ...
                        <p className="text-xs text-slate-400 dark:text-gray-500 font-medium mt-1 uppercase tracking-widest">{activeCases.length} Ambulances En Route</p>
                    </div>

                    <div className="flex-1 overflow-y-auto p-4 space-y-3">
                        {activeCases.map((amb) => {
                            const hName = MOCK_HOSPITALS.find(h => h.id === amb.targetHospital)?.name;
                            ...
                            return (
                                <div
                                    key={amb.id}
                                    onClick={() => setActiveCaseId(isSelected ? null : amb.caseId)}
                  ...
                  `}
                                >
                                    {/* Severity Banner */}
                                    <div className={`absolute top-0 left-0 w-1 h-full ${isCritical ? 'bg-red-500' : 'bg-orange-500'}`}></div>

                                    <div className="flex justify-between items-start pl-2 mb-2">
                                        <div>
                                            <h3 className="text-sm font-black text-white">{amb.caseId}</h3>
                                            <div className="text-[10px] font-bold uppercase tracking-widest text-blue-400">{amb.id}</div>
                                        </div>
                                        <div className="flex flex-col items-end">
                                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded shadow text-white uppercase tracking-widest
                       ...
                       `}>
                                    ...
                                    <div className="pl-2 mt-3 flex justify-between items-end border-t border-slate-700/50 pt-2">
                                        <div>
                                            <div className="text-xs text-slate-400 dark:text-gray-500 font-medium">Destination:</div>
                                            <div className="text-sm font-bold text-slate-300 truncate max-w-[140px]">{hName}</div>
                                        </div>
                                        <div className="text-right">
                                            <div className="text-xs font-bold text-slate-500 dark:text-gray-400 uppercase tracking-widest mb-0.5">ETA</div>
                                            <div className={`text-xl font-black font-mono tracking-tighter ${isSelected ? 'text-yellow-400' : 'text-slate-200'}`}>
                                                {amb.eta}
                                            </div>
                                        </div>
                                    </div>
