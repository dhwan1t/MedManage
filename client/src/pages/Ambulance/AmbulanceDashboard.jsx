...
export default function AmbulanceDashboard() {
    ...
    useEffect(() => {
        ...
        socket.on('case:assigned', (data) => {
            ...
            setActiveCase(data);
            setStatus("ON CALL"); // Automatically set status if a case is pushed
        });
    ...
    const handleStatusChange = async (newStatus) => {
        ...
        try {
            // Mock API call if no real backend
            ...
            // If switching off ON CALL manually, we usually clear the case
            if (newStatus !== "ON CALL" && status === "ON CALL") {
                setActiveCase(null);
            }
    ...
    // Mock assigning a case for demo purposes
    const simulateNewCase = () => {
        const mockCase = {
            ...
            timeAssigned: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
            severityScore: "Pending Vitals", // This would be updated after vitals entry
            notes: "Patient reported severe crushing chest pain radiating to left arm."
        };
    ...
    const getStatusColor = (currentStatus) => {
        switch (currentStatus) {
            case "AVAILABLE": return "bg-green-500 text-white shadow-[0_0_15px_rgba(34,197,94,0.4)]";
            case "ON CALL": return "bg-red-500 text-white shadow-[0_0_15px_rgba(239,68,68,0.4)]";
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    ...
                    <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-xl font-bold tracking-tight">AMB-2047</h1>
                        ...
                        <div className="text-right hidden sm:block">
                            <p className="text-sm font-semibold text-slate-300">Medic Unit 4</p>
                    ...
                    <div className="bg-slate-800 rounded-xl p-1.5 flex transition-colors relative">
                        ...
                        {["AVAILABLE", "ON CALL", "OFF DUTY"].map(s => (
                            <button
                                ...
                                className={`flex-1 py-2.5 px-3 rounded-lg text-sm font-bold transition-all duration-200 ${status === s
                                        ? getStatusColor(s)
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
                ...
                {status === "OFF DUTY" && (
                    <div className="bg-white rounded-2xl p-10 text-center border border-gray-200 shadow-sm mt-8 animate-fadeIn">
                        ...
                        <h2 className="text-2xl font-bold text-gray-900 mb-2">You are Off Duty</h2>
                        <p className="text-gray-500 max-w-sm mx-auto">Change your status to Available to start receiving new emergency dispatches.</p>
                    </div>
                )}
                ...
                {status === "AVAILABLE" && !activeCase && (
                    <div className="bg-white rounded-2xl p-10 text-center border border-green-100 shadow-sm mt-8 animate-fadeIn relative overflow-hidden">
                        {/* Soft pulsing background elements */}
                        ...
                        <div className="relative z-10">
                            ...
                            <h2 className="text-2xl font-bold text-gray-900 mb-2">Awaiting Dispatch</h2>
                            <p className="text-gray-500 max-w-sm mx-auto mb-8">You are available. Any new cases assigned to your unit will appear here automatically.</p>
                    ...
                    <div className="space-y-6 animate-fadeIn">
                        ...
                        <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-xl flex items-center justify-between shadow-sm">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-red-800 font-bold uppercase tracking-wider text-sm">Active Emergency Response</span>
                            </div>
                            <span className="text-sm font-bold text-red-600">Assigned: {activeCase.timeAssigned}</span>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-md border border-gray-200 overflow-hidden">
                            ...
                            <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between md:items-center gap-4">
                                <div>
                                    <h2 className="text-2xl font-bold text-gray-900 border-b-2 border-transparent inline-block">
                                        {activeCase.patientName} <span className="text-gray-400 font-normal ml-2">({activeCase.age}y/o)</span>
                                    </h2>
                                    <p className="text-red-600 font-bold mt-1 text-lg">{activeCase.emergencyType}</p>
                                </div>
                                ...
                                <div className="bg-amber-50 border border-amber-200 px-4 py-2 rounded-xl flex items-center gap-2 self-start md:self-auto">
                                    ...
                                    <div>
                                        <p className="text-xs text-amber-700 font-semibold uppercase tracking-wider">Severity Score</p>
                                        <p className="text-lg font-bold text-amber-800 leading-none">{activeCase.severityScore}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-6 bg-slate-50 border-b border-gray-100">
                                <div className="flex items-start gap-4 mb-4">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-gray-900 text-lg">{activeCase.location}</h3>
                                        <p className="text-blue-600 font-medium text-sm mt-0.5">{activeCase.distance}</p>

                                        <button className="mt-2 text-sm bg-white border border-gray-200 shadow-sm px-3 py-1.5 rounded-lg font-semibold text-gray-700 hover:bg-gray-50 flex items-center gap-1.5 transition-colors">
                                ...
                                <div className="mt-6">
                                    <h4 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">Dispatch Notes</h4>
                            ...
                            <div className="p-6 bg-white flex flex-col sm:flex-row gap-4">
                                <button
                                    onClick={() => navigate('/ambulance/vitals')}
                                ...
                                <button
                                    onClick={() => navigate('/ambulance/recommendations')}
