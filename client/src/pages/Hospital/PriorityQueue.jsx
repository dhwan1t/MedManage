...
const INITIAL_SORTED_QUEUE = [...MOCK_QUEUE].sort(
...
const PriorityQueue = () => {
  ...
  useEffect(() => {
    ...
    socket.on("ambulance:dispatch", (newPatient) => {
      setQueue((prev) => {
        ...
        const newPos = resorted.findIndex(
    ...
    socket.on("patient:vitals_update", (update) => {
      setQueue((prev) => {
        const oldIndex = prev.findIndex(
          (p) => p.patientId === update.patientId,
        );
        ...
        const newIndex = resorted.findIndex(
          (p) => p.patientId === update.patientId,
        );
  ...
  const triggerDemoInterrupt = () => {
    ...
    const socketMockEvent = new CustomEvent("socket_mock_dispatch", {
      detail: demoPatient,
    });
  ...
  const getSeverityBadgeClass = (score) => {
    ...
    if (score >= 40)
      return "bg-yellow-400 text-yellow-900 border-yellow-500 shadow-sm";
    ...
    <div className="min-h-screen bg-white text-gray-900 dark:bg-gray-950 dark:text-white">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <header className="flex flex-col md:flex-row md:justify-between items-start md:items-end gap-4 border-b border-slate-200 dark:border-gray-700 pb-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="text-xs bg-slate-800 text-white px-3 py-1.5 rounded uppercase tracking-widest font-bold hover:bg-slate-700 dark:text-gray-200 transition"
            >
        ...
        <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 flex items-start sm:items-center gap-4 shadow-sm">
          <div className="bg-indigo-100 p-2.5 rounded-lg text-indigo-600 text-xl hidden sm:block">
            🧠
          </div>
          <p className="text-indigo-900 text-sm font-medium leading-relaxed">
            <strong className="font-bold">AI Triage Active:</strong> This queue
            is managed by real-time AI scoring (MEWS + Shock Index + GCS). A
            patient who arrives later but is more critical will automatically
            jump to the top and be treated first.
          </p>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="col-span-2 flex justify-center">
                      <div
                        className={`w-12 h-12 flex items-center justify-center rounded-full font-black text-lg border-2 border-white shadow-sm transition-colors duration-700 ${getSeverityBadgeClass(patient.severity.score)}`}
                      >
            ...
            {selectedPatient ? (
              <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-slate-100 dark:border-gray-800 p-6 flex-1 flex flex-col h-full overflow-y-auto">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h2 className="text-2xl font-black text-slate-800 dark:text-gray-200">
                      {selectedPatient.patientId}
                    </h2>
                    <p className="text-slate-500 dark:text-gray-400 font-medium">
                      {selectedPatient.age} yo{" "}
                      {selectedPatient.gender === "M" ? "Male" : "Female"} •
                      ETA: {selectedPatient.eta}
                    </p>
                  </div>
                  <div
                    className={`px-4 py-1.5 rounded-lg font-bold text-sm uppercase tracking-wider ${getSeverityBadgeClass(selectedPatient.severity.score)}`}
                  >
                ...
                <div className="space-y-6 flex-1">
                    ...
                    <div className="bg-red-50 border border-red-100 rounded-xl p-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedPatient.severity.flags.map((flag) => (
                          <span
                            key={flag}
                            className="bg-red-100 text-red-800 text-xs font-bold px-2.5 py-1 rounded shadow-sm border border-red-200"
                          >
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest mb-2">
                      Chief Complaint
                    </h3>
                    <p className="text-lg font-bold text-slate-800 dark:text-gray-200 mb-5">
                      {selectedPatient.chiefComplaint}
                    </p>

                    <h3 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest mb-3">
                      Live Vitals
                    </h3>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="border border-slate-100 dark:border-gray-800 rounded-xl p-3 bg-slate-50 dark:bg-gray-900/50">
                        ...
                        <div
                          className={`text-xl font-black ${selectedPatient.vitals.heartRate > 100 || selectedPatient.vitals.heartRate < 60 ? "text-red-500" : "text-slate-800 dark:text-gray-200"}`}
                        >
                      ...
                      <div className="border border-slate-100 dark:border-gray-800 rounded-xl p-3 bg-slate-50 dark:bg-gray-900/50">
                        ...
                        <div
                          className={`text-xl font-black ${selectedPatient.vitals.systolic < 90 || selectedPatient.vitals.systolic > 160 ? "text-red-500" : "text-slate-800 dark:text-gray-200"}`}
                        >
                          {selectedPatient.vitals.systolic}/
                          {selectedPatient.vitals.diastolic}
                        </div>
                      </div>
                      <div className="border border-slate-100 dark:border-gray-800 rounded-xl p-3 bg-slate-50 dark:bg-gray-900/50">
                        <div className="text-xs text-slate-500 dark:text-gray-400 font-bold uppercase mb-1">
                        ...
                        <div
                          className={`text-xl font-black ${selectedPatient.vitals.oxygenSat < 94 ? "text-red-500" : "text-slate-800 dark:text-gray-200"}`}
                        >
                      ...
                      <div className="border border-slate-100 dark:border-gray-800 rounded-xl p-3 bg-slate-50 dark:bg-gray-900/50">
                        ...
                        <div
                          className={`text-xl font-black ${selectedPatient.vitals.temperature > 38 || selectedPatient.vitals.temperature < 36 ? "text-red-500" : "text-slate-800 dark:text-gray-200"}`}
                        >
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedPatient.requiredResources.map((res) => (
                        <span
                          key={res}
                          className="bg-slate-100 dark:bg-gray-800 text-slate-700 dark:text-gray-300 text-xs font-bold px-3 py-1.5 rounded border border-slate-200 dark:border-gray-700"
                        >
                          {res}
                        </span>
                      ))}
              ...
              <div className="bg-slate-100 dark:bg-gray-800 border-2 border-dashed border-slate-200 dark:border-gray-700 rounded-2xl flex-1 flex flex-col items-center justify-center text-slate-400 dark:text-gray-500 p-8 text-center h-full">
                ...
                <p className="text-sm font-medium mt-1">
                  Click a row in the queue to view clinical details and allocate
                  resources.
                </p>
              </div>
            )}
