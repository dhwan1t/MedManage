...
const BedManagement = () => {
    ...
    const [otTheatres, setOtTheatres] = useState([
        { id: 'OT-1', name: 'Theatre 1', status: 'AVAILABLE', equipment: 'General', surgeon: 'Dr. Adams' },
        { id: 'OT-2', name: 'Theatre 2', status: 'OCCUPIED', equipment: 'Cardiac', surgeon: 'Dr. Stevens' },
        { id: 'OT-3', name: 'Theatre 3', status: 'RESERVED', equipment: 'Neuro', surgeon: 'Dr. Lee' },
        { id: 'OT-4', name: 'Theatre 4', status: 'AVAILABLE', equipment: 'Ortho', surgeon: 'Dr. Patel' }
    ]);
    ...
    const getStatusColor = (status) => {
        ...
        if (status === 'RESERVED') return 'bg-yellow-100 border-yellow-300 text-yellow-800';
    ...
    const confirmReservation = () => {
        ...
        applyStatusChange(pendingReservation.type, pendingReservation.item.id, pendingReservation.newStatus, selectedPatient);
    ...
    const applyStatusChange = (type, id, newStatus, patientId = null) => {
        ...
        if (type === 'ICU') {
            setIcuBeds(prev => prev.map(b => b.id === id ? { ...b, status: newStatus } : b));
        } else if (type === 'OT') {
            setOtTheatres(prev => prev.map(t => t.id === id ? { ...t, status: newStatus } : t));
        } else if (type === 'ER') {
            setErBays(prev => prev.map(e => e.id === id ? { ...e, status: newStatus } : e));
        }
        ...
        <div className="min-h-screen bg-white text-gray-900 dark:bg-gray-950 dark:text-white">
            <div className="max-w-6xl mx-auto space-y-6">
                ...
                <header className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-gray-800 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div>
                        <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Resource Management</h1>
                        <p className="text-sm font-medium text-slate-500 dark:text-gray-400 mt-1">Update hospital bed and theatre availability.</p>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-4 bg-slate-50 dark:bg-gray-900 p-3 rounded-xl border border-slate-200 dark:border-gray-700">
                            <span className="font-bold text-slate-700 dark:text-gray-300 whitespace-nowrap">General Beds Available:</span>
              ...
              `}
                        >
                            {tab} {tab === 'General' ? 'Beds' : tab === 'ICU' ? 'Beds' : tab === 'OT' ? 'Theatres' : 'Bays'}
                        </button>
                    ))}
                </div>
                    ...
                    {activeTab === 'General' && (
                        <div>
                            <div className="flex justify-between items-end mb-6">
                                <h2 className="text-lg font-bold text-slate-800 dark:text-gray-200">General Ward Occupancy Matrix</h2>
                                <div className="flex gap-4 text-xs font-bold text-slate-500 dark:text-gray-400 uppercase">
                                    <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-green-400"></span> Available</span>
                                    <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-red-400"></span> Occupied</span>
                                </div>
                            </div>
                    ...
                    {activeTab === 'ICU' && (
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                            {icuBeds.map(bed => (
                                ...
                                >
                                    <div className={`absolute top-3 right-3 w-3 h-3 rounded-full shadow-sm border border-black/10 ${getStatusDotColor(bed.status)}`}></div>
                                    <h3 className="font-bold text-lg mb-1">{bed.name}</h3>
                                    <div className="text-xs font-bold uppercase tracking-wider opacity-80">{bed.status}</div>
                                </button>
                            ))}
                        </div>
                    )}
                            ...
                            {otTheatres.map(theatre => (
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <div className={`w-3.5 h-3.5 rounded-full shadow-sm border border-black/10 ${getStatusDotColor(theatre.status)}`}></div>
                                    </div>

                                    <div className="bg-white dark:bg-gray-900/50 rounded-lg p-3 space-y-1 my-2 border border-black/5">
                                        <p className="text-sm"><strong>Type:</strong> {theatre.equipment}</p>
                                        <p className="text-sm"><strong>Surgeon:</strong> {theatre.surgeon}</p>
                                    </div>

                                    <div className="mt-auto pt-2 border-t border-black/10 text-xs font-bold uppercase tracking-widest flex items-center gap-2">
                                        Status: <span className="underline decoration-2 underline-offset-4">{theatre.status}</span>
                                    </div>
                                </button>
                            ))}
                        </div>
                    ...
                    {activeTab === 'ER' && (
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                            {erBays.map(bay => (
                                ...
                                >
                                    <div className={`absolute top-4 right-4 w-3.5 h-3.5 rounded-full shadow-sm border border-black/10 ${getStatusDotColor(bay.status)}`}></div>
                                    <h3 className="font-extrabold text-xl mb-1">{bay.name}</h3>
                                    <div className="text-xs font-bold uppercase tracking-wider opacity-80 mt-2">{bay.status}</div>
                                </button>
                            ))}
                        </div>
                    )}
            ...
            {modalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-slate-200 dark:border-gray-700 max-w-md w-full p-6 animate-in fade-in zoom-in-95 duration-200">
                        <h2 className="text-xl font-bold text-slate-800 dark:text-gray-200 mb-2">Reserve {pendingReservation?.item?.name}?</h2>
                        <p className="text-slate-500 dark:text-gray-400 text-sm mb-6">Assign this resource to an incoming patient to mark it as reserved.</p>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                >
                                    <option value="" disabled>-- Select Incoming Ambulance --</option>
                                    {MOCK_INCOMING.map(pt => (
                                        <option key={pt.id} value={pt.id}>{pt.id} - {pt.name} (ETA: {pt.eta})</option>
                                    ))}
                                </select>
                            </div>
                            ...
                            <div className="flex gap-3 pt-4 border-t border-slate-100 dark:border-gray-800">
                                <button
                                    onClick={() => { setModalOpen(false); setPendingReservation(null); }}
